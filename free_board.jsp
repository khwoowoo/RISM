...
<div class="wrapper">
	...
	<span class="pageTitle" style="text-indent: 30px;"> 자유게시판 </span>
...
ResultSet rs = null;
int it=0;
String imgpath = getServletContext().getRealPath("uploadimgs");
int resultcnt = 0;
...
try {
	...
	while(rs.next())
	{
		...
		%>
		<div class="post_it" onclick="location.href='free_board_show.jsp?id=<%=id%>'">
		<%
		if(fp != null && (fp.contains("jpg") || fp.contains("jpeg") || fp.contains("png"))){
			%><div style="width:100%; height:66%; background-image:url('uploadimgs/<%=fp%>'); background-size: cover;"></div><%
		}
		else{
			%><div style="width:100%; height:66%; background-image:url('images/image.jpg'); background-size: cover;"></div><%
		}
		%>
