...
// INSERT INTO table-name : 지정 column-name 순 번으로, row-data를 생성합니다. 
    return db.execute(
      'INSERT INTO address_tbl (student_id, name, address, e_mail) VALUES (?, ?, ?, ?)',
      [this.id, this.name, this.address, this.email]
    );
  }
// SELECT column-name : 데이터 조회에 사용됩니다. 
  static fetchAll() {
    return db.execute('SELECT * FROM address_tbl');
  }
// WHRER  : 데이터에 대해 조건을 겁니다. 
  static findById(id) {
    return db.execute('SELECT * FROM address_tbl WHERE products.id = ?', [id]);
  }
};

console.log(module.exports.fetchAll());